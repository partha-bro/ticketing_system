...
$sql1="SELECT max(ticket_id) FROM ost_ticket";
		$sql = "SELECT ost_ticket.number Ticket_no,
				...
				ost_ticket__cdata.subject Subject,
                		CONCAT(ost_staff.firstname,' ',ost_staff.lastname) Assign_staff,
				(select CONCAT(firstname,' ',lastname) from ost_staff where ost_staff.staff_id=ost_ticket.staff_id) Closed_By,
				ost_ticket_status.state State,
				date(ost_ticket.closed) Closed,
				fnStripTags(ost_thread_entry.body) as Comment
                FROM ost_ticket
		INNER JOIN ost_user ON ost_ticket.user_id = ost_user.id
                ...
                INNER JOIN ost_thread ON ost_ticket.ticket_id = ost_thread.object_id
                INNER JOIN ost_thread_entry ON ost_thread.id = ost_thread_entry.thread_id
                INNER JOIN ost_thread_event ON ost_thread.id = ost_thread_event.thread_id
                INNER JOIN ost_staff ON ost_staff.staff_id = ost_thread_event.staff_id
    WHERE ost_thread_entry.thread_id in (SELECT id FROM ost_thread WHERE object_type = 'T' AND object_id ='$no')
	AND (ost_thread_event.state='assigned')
...
while($no<=$count_last_id){
	...
	AND (ost_thread_event.state='assigned')
	AND (ost_thread.object_type = 'T')
	AND (ost_ticket_status.state = 'closed')
	AND (ost_ticket.closed BETWEEN '$firstdate' AND '$lastdate')
	order by ost_thread_entry.id desc 
	limit 1");
		while ($field = db_fetch_row($res)) {
			$rows[] = $field;
