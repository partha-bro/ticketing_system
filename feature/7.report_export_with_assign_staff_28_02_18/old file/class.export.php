...
$sql1="SELECT max(ticket_id) FROM ost_ticket";		
		$sql = "SELECT ost_ticket.number Ticket_no, 
				ost_user__cdata.account as Account_code, 
				ost_user__cdata.category Category, 
				ost_user.name Name, ost_user__cdata.address as Address, 
				ost_user__cdata.branch as Branch, 
				ost_ticket__cdata.vender as Vendor_Complaint_No , 
				ost_organization.name Cluster, 
				date(ost_ticket.created) as Created, 
				ost_ticket__cdata.calllog as Name_of_complaint,
				ost_ticket__cdata.mobno as Mobile_No, 
				ost_help_topic.topic Help_Topic, 
				ost_ticket__cdata.subject Subject, 
				ost_staff.firstname Assign_Staff, 
				ost_ticket_status.state State,    
				date(ost_ticket.closed) Closed, 
				fnStripTags(ost_thread_entry.body) as Comment FROM ost_ticket 
...
$no = 1;
while($no<=$count_last_id){		
		$res = db_query("SELECT ost_ticket.number Ticket_no, 
					ost_user__cdata.account as Account_code, 
					ost_user__cdata.category Category, 
					ost_user.name Name, ost_user__cdata.address as Address, 
					ost_user__cdata.branch as Branch, 
...
while($no<=$count_last_id){		
		$res = db_query("SELECT ost_ticket.number Ticket_no, 
				...
				ost_user__cdata.category Category, 
				ost_user.name Name, 
				ost_user__cdata.address as Address, 
				ost_user__cdata.branch as Branch, 
				ost_ticket__cdata.vender as Vendor_Complaint_No , 
				ost_organization.name Cluster, 
				date(ost_ticket.created) as Created, 
