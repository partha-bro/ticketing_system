...
static  function exportTickets($filename, $how='csv') {
        Http::download($filename, "text/$how");        
		$headers = array(
						'Name',
						'Address',
		...
		$res = db_query("SELECT ost_user.name Name, ost_user__cdata.address as Address, ost_user_email.address Email, ost_ticket.number Ticket_no, ost_user__cdata.account as Account_code, ost_ticket_status.state State, ost_organization.name Cluster, ost_user__cdata.category Category, ost_ticket__cdata.subject Subject,ost_help_topic.topic Help_Topic,ost_staff.firstname Assign_Staff,date(ost_ticket.created) as Created, date(ost_ticket.closed) Closed FROM ost_ticket 
