...
if($_GET['export']){
	...
	if($_REQUEST['a'] == 'export_custom') {
		if ($days <= "365"){
			...
			if (!Export::exportTickets($param, "tickets-$ts.csv", 'csv'))
				$errors['err'] = __('Unable to dump query results.')
					.' '.__('Internal error occurred');
			}
			elseif ($param['ticket_status'] == 'open'){
				if (!Export::exportOpenTickets($param, "tickets-$ts.csv", 'csv'))
				$errors['err'] = __('Unable to dump query results.')
					.' '.__('Internal error occurred');
			}
			else{
				if (!Export::exportCloseTickets($param, "tickets-$ts.csv", 'csv'))
				$errors['err'] = __('Unable to dump query results.')
					.' '.__('Internal error occurred');
		}
		}else{
			echo '<script type="text/javascript">alert("1 year is maximum duration.Please select date in between 1 year.");</script>';
			}
	}	
}
...
<form action="" method="GET">
	<div>
	<input type="text" id="fromdate" name="fdate" value="" required>
	<input type="text" id="todate" name="ldate" value="" required>
