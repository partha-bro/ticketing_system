...
static  function exportTickets($param, $filename, $how='csv', $count_last_id) {
	$firstdate = $param['fdate']; 
	$lastdate = $param['ldate'];
...
$no = 1;
while($no<=$count_last_id){
$sql1="SELECT max(ticket_id) FROM ost_ticket";		
	$sql = "SELECT ost_ticket.number Ticket_no, ost_user__cdata.account as Account_code, ost_user__cdata.category Category, ost_user.name Name, ost_user__cdata.address as Address, ost_user__cdata.branch as Branch, ost_ticket__cdata.vender as Vendor_Complaint_No , ost_organization.name Cluster, date(ost_ticket.created) as Created, ost_ticket__cdata.calllog as Name_of_complaint, ost_ticket__cdata.mobno as Mobile_No, ost_help_topic.topic Help_Topic, ost_ticket__cdata.subject Subject, ost_staff.firstname Assign_Staff, ost_ticket_status.state State,    date(ost_ticket.closed) Closed, fnStripTags(ost_thread_entry.body) as Comment FROM ost_ticket 
	INNER JOIN ost_user ON ost_ticket.user_id = ost_user.id 
	INNER JOIN ost_user_email ON ost_ticket.user_id = ost_user_email.user_id 
	INNER JOIN ost_ticket_status ON ost_ticket.status_id = ost_ticket_status.id 
	INNER JOIN ost_organization ON ost_user.org_id = ost_organization.id 
	INNER JOIN ost_user__cdata ON ost_user.id = ost_user__cdata.user_id 
    ...
    WHERE ost_thread_entry.id in (SELECT MAX(id) FROM ost_thread_entry WHERE thread_id='$no' )
	AND ost_ticket.created BETWEEN '$firstdate' AND '$lastdate'
	ORDER BY ost_user__cdata.category";	
	$res = db_query($sql);	
		while ($field = db_fetch_row($res)) {			
			$rows[] = $field;		
...
static  function exportOpenTickets($param, $filename, $how='csv', $count_last_id) {
	$firstdate = $param['fdate'];
	$lastdate = $param['ldate'];
        Http::download($filename, "text/$how");        
		$headers = array(
...
$no = 1;
while($no<=$count_last_id){		
		$res = db_query("SELECT ost_ticket.number Ticket_no, ost_user__cdata.account as Account_code, ost_user__cdata.category Category, ost_user.name Name, ost_user__cdata.address as Address, ost_user__cdata.branch as Branch, ost_ticket__cdata.vender as Vendor_Complaint_No , ost_organization.name Cluster, date(ost_ticket.created) as Created, ost_ticket__cdata.calllog as Name_of_complaint, ost_ticket__cdata.mobno as Mobile_No, ost_help_topic.topic Help_Topic, ost_ticket__cdata.subject Subject, ost_staff.firstname Assign_Staff, ost_ticket_status.state State,    date(ost_ticket.closed) Closed , fnStripTags(ost_thread_entry.body) as Comment FROM ost_ticket  INNER JOIN ost_user ON ost_ticket.user_id = ost_user.id INNER JOIN ost_user_email ON ost_ticket.user_id = ost_user_email.user_id INNER JOIN ost_ticket_status ON ost_ticket.status_id = ost_ticket_status.id INNER JOIN ost_organization ON ost_user.org_id = ost_organization.id INNER JOIN ost_user__cdata ON ost_user.id = ost_user__cdata.user_id INNER JOIN ost_ticket__cdata ON ost_ticket.ticket_id = ost_ticket__cdata.ticket_id INNER JOIN ost_help_topic ON ost_ticket.topic_id = ost_help_topic.topic_id INNER JOIN ost_staff ON ost_ticket.staff_id = ost_staff.staff_id INNER JOIN ost_thread_entry ON ost_ticket.ticket_id = ost_thread_entry.thread_id
    WHERE ost_thread_entry.id in (SELECT MAX(id) FROM ost_thread_entry WHERE thread_id='$no' ) AND ost_ticket_status.state = 'open' AND ost_ticket.created BETWEEN '$firstdate' AND '$lastdate' ORDER BY ost_user__cdata.category");
		while ($field = db_fetch_row($res)) {
			$rows[] = $field;		
...
static  function exportCloseTickets($param, $filename, $how='csv', $count_last_id) {
	$firstdate = $param['fdate'];
	$lastdate = $param['ldate'];
        Http::download($filename, "text/$how");        
		$headers = array(
...
while($no<=$count_last_id){		
		$res = db_query("SELECT ost_ticket.number Ticket_no, ost_user__cdata.account as Account_code, ost_user__cdata.category Category, ost_user.name Name, ost_user__cdata.address as Address, ost_user__cdata.branch as Branch, ost_ticket__cdata.vender as Vendor_Complaint_No , ost_organization.name Cluster, date(ost_ticket.created) as Created, ost_ticket__cdata.calllog as Name_of_complaint, ost_ticket__cdata.mobno as Mobile_No, ost_help_topic.topic Help_Topic, ost_ticket__cdata.subject Subject, ost_staff.firstname Assign_Staff, ost_ticket_status.state State,    date(ost_ticket.closed) Closed, fnStripTags(ost_thread_entry.body) as Comment FROM ost_ticket  INNER JOIN ost_user ON ost_ticket.user_id = ost_user.id INNER JOIN ost_user_email ON ost_ticket.user_id = ost_user_email.user_id INNER JOIN ost_ticket_status ON ost_ticket.status_id = ost_ticket_status.id INNER JOIN ost_organization ON ost_user.org_id = ost_organization.id INNER JOIN ost_user__cdata ON ost_user.id = ost_user__cdata.user_id INNER JOIN ost_ticket__cdata ON ost_ticket.ticket_id = ost_ticket__cdata.ticket_id INNER JOIN ost_help_topic ON ost_ticket.topic_id = ost_help_topic.topic_id INNER JOIN ost_staff ON ost_ticket.staff_id = ost_staff.staff_id INNER JOIN ost_thread_entry ON ost_ticket.ticket_id = ost_thread_entry.thread_id
    WHERE ost_thread_entry.id in (SELECT MAX(id) FROM ost_thread_entry WHERE thread_id='$no' ) AND ost_ticket_status.state = 'closed' AND ost_ticket.closed BETWEEN '$firstdate' AND '$lastdate' ORDER BY ost_user__cdata.category");
		while ($field = db_fetch_row($res)) {
			$rows[] = $field;		
