...
if($result_row = mysqli_query($link, $sql_row)){
    if(mysqli_num_rows($result_row) > 0){
	 while($row_row = mysqli_fetch_array($result_row)){
		$data = $row_row[0];
$sql = "SELECT * FROM ost_ticket inner join ost_user__cdata on ost_ticket.user_id=ost_user__cdata.user_id where ost_ticket.status_id='1' and ost_user__cdata.branch ='$data'";
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
	 if($row = mysqli_fetch_array($result)){
		$count = mysqli_num_rows($result);
